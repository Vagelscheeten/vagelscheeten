...
interface SpendenZuweisungModalProps {
  open: boolean;
  onClose: () => void;
...
export function SpendenZuweisungModal({ 
  open, 
  onClose, 
  bedarf,
  onSave,
  verfuegbareMenge
}: SpendenZuweisungModalProps) {
  ...
  useEffect(() => {
    const fetchData = async () => {
      ...
      try {
        ...
        const zugewieseneIdentifier = bedarf.zuteilungen.map(z => z.kind_identifier);
  ...
  const handleSubmit = async () => {
    if (selectedKindIds.length === 0) {
      toast.error('Bitte wähle mindestens ein Kind aus');
      return;
    }
    ...
    if (menge * selectedKindIds.length > verfuegbareMenge) {
    ...
    try {
      ...
      for (const kindId of selectedKindIds) {
        // Das ausgewählte Kind finden
        const selectedKind = kinder.find(kind => kind.id === kindId);
        if (!selectedKind) {
          console.warn(`Kind mit ID ${kindId} nicht gefunden`);
          continue;
        }
        ...
        if (existingRueckmeldung) {
          toast.error(`${kindIdentifier} ist bereits für diese Essensspende eingetragen`);
          continue;
        }
      ...
      if (zuweisungen.length > 0) {
        ...
        toast.success(`${zuweisungen.length} Essensspende(n) erfolgreich zugewiesen`);
        onSave();
      }
    } catch (error: any) {
  ...
  const toggleKindSelection = (kindId: string) => {
  ...
  return (
    <Dialog open={open} onOpenChange={onClose}>
      <DialogContent className="sm:max-w-[600px]">
        <DialogHeader>
          <DialogTitle>
            Kinder für "{bedarf?.titel}" zuweisen
          </DialogTitle>
        </DialogHeader>
        ...
        <div className="mt-4 space-y-4">
          ...
          <div className="space-y-2">
            <div className="flex justify-between items-center">
              <Label>Kinder auswählen</Label>
              <Badge variant="outline">
                {selectedKindIds.length} ausgewählt
              </Badge>
            </div>
            
            <div className="border rounded-md p-4 max-h-60 overflow-y-auto space-y-2">
              {isLoading ? (
                <div className="text-center py-4">Kinder werden geladen...</div>
              ) : verfuegbareKinder.length === 0 ? (
                <div className="text-center py-4">Keine verfügbaren Kinder gefunden</div>
              ) : (
                verfuegbareKinder.map(kind => (
                  <div key={kind.id} className="flex items-center space-x-2">
                    <Checkbox 
                      id={`kind-${kind.id}`} 
                      checked={selectedKindIds.includes(kind.id)}
                      onCheckedChange={() => toggleKindSelection(kind.id)}
                    />
                    <Label htmlFor={`kind-${kind.id}`} className="cursor-pointer flex-grow">
          ...
          <div className="space-y-2">
            <Label htmlFor="menge">Menge pro Kind</Label>
            <Input
              id="menge"
              type="number"
              min="1"
        ...
        <DialogFooter className="mt-6">
          ...
          <Button 
            ...
            disabled={isSubmitting || selectedKindIds.length === 0 || menge <= 0 || menge * selectedKindIds.length > verfuegbareMenge}
          >
