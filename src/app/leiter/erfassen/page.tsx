...
export default async function ErgebnisErfassenPage() {
  ...
  // Spiele für die Klassenstufe und das aktuelle Event laden
  const { data: spiele } = await supabase
    .from('spiele')
    .select(`
      *,
      klassenstufe_spiele!inner(*),
      event_spiele!inner(event_id)
    `)
    .eq('klassenstufe_spiele.klassenstufe_id', spielgruppe.klassenstufe_id)
        ...
        <CardHeader>
          <div className="flex justify-between items-start gap-4">
            <div className="space-y-1.5">
              ...
              <CardDescription>
                {spielgruppe.klassenstufen?.name || 'Unbekannte Klassenstufe'}
              </CardDescription>
            </div>
